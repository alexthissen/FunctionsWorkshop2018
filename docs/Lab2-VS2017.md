...
<img src="images/FileNewProject.png" height="250"/>
...
<img src="images/NewFunctionVS2017.png" height="400"/>
...
If all went well, you should find a new guid entry in the blob storage ```azurefunctions-qrcode-images```. Open it to see if it contains your QR code. In the case that it doesn't work, use Visual Studio for debugging. The console window might also contains some logging information to help diagnose the error.

## <a name="4"></a>4. Advanced bindings

In the final example for now, you will create a function that imports the contents of a queue message. The message contains JSON content with a list of high scores. The values need to be imported into a Azure Storage table.
...
Add code to your solution for two POCO classes. The first named ```HighScoreEntry``` is used to deserialize the JSONcontent of the :
...
- Take one of the samples from the Azure portal when creating a new function there and convert it to .NET. This means you have to apply the correct attributes on the function.
